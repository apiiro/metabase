...
(def ^:private app-defaults
  ...
  {:mb-run-mode                     "prod"
   ...
   ;; Credentials for the bundled read-only Sample Database H2 file. These defaults match the GUEST account
   ;; that is baked into the shipped `sample-database.db.mv.db` artifact; override only if you rebuild that
   ;; artifact with different credentials.
   :mb-sample-database-user         "GUEST"
   :mb-sample-database-password     "guest"})
